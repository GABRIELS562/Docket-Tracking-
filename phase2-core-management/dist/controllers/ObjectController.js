...
class ObjectController {
    ...
    async listObjects(req, res) {
        try {
            const { page = 1, limit = 50, type, status = 'active', location_id, assigned_to_id } = req.query;
      ...
      `;
            ...
            server_1.logger.error('Error listing objects:', error);
    ...
    async getObject(req, res) {
      ...
      `, [id]);
            ...
            server_1.logger.error('Error getting object:', error);
    ...
    async createObject(req, res) {
        try {
            ...
            const rfidTagId = req.body.rfid_tag_id || `TAG-${(0, uuid_1.v4)().substring(0, 8)}`;
            const result = await this.db.query(`
        INSERT INTO objects (
      ...
      `, [result.rows[0].id, userId, JSON.stringify(result.rows[0])]);
            ...
            server_1.logger.error('Error creating object:', error);
    ...
    async updateObject(req, res) {
      ...
      `, [id, userId, JSON.stringify(currentResult.rows[0]), JSON.stringify(result.rows[0])]);
            ...
            server_1.logger.error('Error updating object:', error);
    ...
    async deleteObject(req, res) {
      ...
      `, [id, userId]);
            ...
            server_1.logger.error('Error deleting object:', error);
    ...
    async searchObjects(req, res) {
      ...
      `;
            ...
            server_1.logger.error('Error searching objects:', error);
            res.status(500).json({ success: false, error: 'Failed to search objects' });
        }
    }
    async assignObject(req, res) {
        try {
            const { id } = req.params;
            ...
            const result = await this.db.transaction(async (client) => {
                ...
                const current = await client.query('SELECT assigned_to_id FROM objects WHERE id = $1', [id]);
        ...
        `, [id, userId, notes]);
            ...
            server_1.logger.error('Error assigning object:', error);
            res.status(500).json({ success: false, error: 'Failed to assign object' });
        }
    }
    async moveObject(req, res) {
        try {
            const { id } = req.params;
            ...
            const result = await this.db.transaction(async (client) => {
                ...
                const current = await client.query('SELECT current_location_id FROM objects WHERE id = $1', [id]);
                if (current.rows.length === 0) {
                    throw new Error('Object not found');
                }
                // Update location
        ...
        `, [id, userId, current.rows[0].current_location_id, location_id, notes]);
            ...
            server_1.logger.error('Error moving object:', error);
            res.status(500).json({ success: false, error: 'Failed to move object' });
        }
    }
    async tagObject(req, res) {
        try {
            const { id } = req.params;
            const { tags } = req.body;
            const result = await this.db.query(`
      ...
      `, [tags, id]);
            ...
            server_1.logger.error('Error tagging object:', error);
    ...
    async getObjectHistory(req, res) {
      ...
      `, [id]);
            ...
            server_1.logger.error('Error getting object history:', error);
    ...
    async getChainOfCustody(req, res) {
      ...
      `, [id]);
            ...
            server_1.logger.error('Error getting chain of custody:', error);
            res.status(500).json({ success: false, error: 'Failed to get chain of custody' });
        }
    }
    async getObjectTypes(_req, res) {
        try {
            const result = await this.db.query(`
      ...
      `);
            ...
            server_1.logger.error('Error getting object types:', error);
            res.status(500).json({ success: false, error: 'Failed to get object types' });
        }
    }
    async getObjectStats(_req, res) {
        try {
      ...
      `);
            ...
            server_1.logger.error('Error getting object stats:', error);
