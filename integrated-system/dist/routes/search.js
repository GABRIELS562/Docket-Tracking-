...
router.get('/', async (req, res) => {
    try {
      ...
      SELECT 'object' as type, id, object_code as code, name, description
      FROM objects 
      WHERE name ILIKE $1 OR object_code ILIKE $1 OR description ILIKE $1
      LIMIT 10
    `, [`%${searchTerm}%`]);
        const personnel = await db.query(`
      SELECT 'personnel' as type, id, employee_id as code, 
             first_name || ' ' || last_name as name, department as description
      FROM personnel 
      WHERE first_name ILIKE $1 OR last_name ILIKE $1 OR employee_id ILIKE $1
      LIMIT 10
    `, [`%${searchTerm}%`]);
        const locations = await db.query(`
      SELECT 'location' as type, id, location_code as code, location_name as name, description
      FROM locations 
      WHERE location_name ILIKE $1 OR location_code ILIKE $1 OR description ILIKE $1
      LIMIT 10
    `, [`%${searchTerm}%`]);
        res.json({
            success: true,
            data: {
                ...
                total: objects.rows.length + personnel.rows.length + locations.rows.length
            }
        });
    }
    catch (error) {
